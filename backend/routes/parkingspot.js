...
router.get('/:stationId', async (req, res) => {
    ...
    DatabaseService.executeSelectionQuery({
        text: 'SELECT ps.spot_id, ps.station_id, ps.spot_number, ARRAY_AGG(bc.category_id) AS categories FROM ParkingSpot ps LEFT JOIN ParkingSpotCategory psc ON ps.spot_id = psc.spot_id LEFT JOIN BikeCategory bc ON psc.category_id = bc.category_id WHERE ps.station_id = $1 GROUP BY ps.spot_id',
        values: [stationId]
    })
        .then(results => res.status(200).json(results))
        .catch(e => {
...
router.delete('/spot/:spotId/:bikeId', async (req, res) => {
    ...
    DatabaseService.executeUpdateQuery({
        text: 'UPDATE Bike set assigned_to = NULL, station_id = NULL WHERE bike_id = $1',
        values: [bikeId]
    })
        .then(result => {
            DatabaseService.executeDeleteQuery({
                text: 'DELETE FROM parkingspotcategory WHERE spot_id = $1',
                values: [spotId]
            })
                .then(result => {
                    DatabaseService.executeDeleteQuery({
                        text: 'DELETE FROM parkingspot WHERE spot_id = $1',
                        values: [spotId]
                    })
